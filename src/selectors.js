export const form = document.querySelector('form');
export const inputField = document.querySelector('form > input');
export const output = document.querySelector('.output');
export const h3 = document.querySelector('h3');
export const hourlyDiv = document.querySelector('.hourlyDiv');
export const main = document.querySelector('main');
export const logo = document.querySelector('div.logo');
export const detailsDiv = document.querySelector('div.card.detailsDiv');
export const dailyDiv = document.querySelector('div.card.dailyDiv');
export const body = document.querySelector('body');